...
> The number $x_p$ ($0 \leq p \leq 1$) is called the $p^{th}$ quantile of a random variable $X$ if: <br>
- Continuous: $P(X\leq x_p) = p$<br>
...
- `Interpretation`: Fail to Reject $H_0$. There is evidence to support that the passing rate of the course is not significantly different from $75\%$ $p-value=0.787$; $95\%CI (0.27,0.79)$. 
...
3. Determine critical values (rejection region): t = `r {17-qbinom(0.05,17,0.5)}`
...
- If no ties: 
$ T^+ = \sum (R_i \text{where} D_i \text{is positive}) \sim$. Tables for exact lower quantiles, under $H_0$, exist. The upper quantile is given: $w_p = n(n+1)/2 - w_{1-p}$. We will use R.

- If there are ties or $n>50$: $ T = \frac{\sum_{i=1}^n R_i}{\sqrt{\sum_{i=1}^n R_i^2}}\sim$ Standard normal distribution, under $H_0$.
