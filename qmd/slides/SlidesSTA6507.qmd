...
-   **Alternative Hypothesis** - $H_1$: $H_0$ is rejected. It is the
    ...
    quality of the product or service is unsatis- factory" or
    \the process is out of control".
...
`1.DATA:` The sample consists of outcomes of $n$ independent trails.
Each trail has two outcomes (mutually exclusive= they cannot both occur
at same time). Let consider $O_1$ to be $\#$ of observations in class1
and $O_2$ to be the $\#$ of observations in class2. We have $n=O_1+O_2$.

`2.Assumptions:` - $n$ observation are independent. - Each trial has
probability $p$ of resulting in the outcome $O_1$, where $p$ is the same
for all $n$ trials.

`3.Test Statistic:` Since we are interested in the probability of an
outcome to be in class1, we will let the test statistic $T$ be
`the number of times the outcomes is in class1`. Therefore, the test
statistic follows the binomial distribution (null distribution) with $n$
and $\hat{p}$ (the hypothesized proportion) parameters.

## Binomial Test (Estimation of $p$)

`4.Hypothesis: Two-tailed test` $$ H_0: p=p^* $$ $$H_1: p \neq p^* $$\
...
`4.Hypothesis: Upper-tailed test` $$ H_0: p \leq p^* $$
$$H_1: p > p^* $$ The rejection region is defined by $t$ as follows:
...
`4.Hypothesis: Lower-tailed test` $$ H_0: p \geq p^* $$
$$H_1: p < p^* $$
...
::: r-fit-text
We would like to know whether the passing rate for a course is $75\%$.
We have a sample of $n=15$ students taking the course and only 8 passed.
Level of significance is $5\%$. Test the appropriate hypothesis as
follows:

1.  Define the null and alternative hypotheses <br> `Answer:`
    $$ H_0: p=0.75 $$ $$H_1: p \neq 0.75 $$
2.  Find the Test statistic observed and null distribution<br> `Answer:`
    $T_{obs}=8$ and $T\sim bin(15,0.75)$
3.  Determine critical values (rejection region) <br> `Answer:`
    t_1=`r {qbinom(0.025,15,0.75)}` and t_2=`r {qbinom(0.975,15,0.75)}`
4.  Find P-value<br> `Answer:`
    p-value=`r {2*min(pbinom(8,15,0.75),pbinom(13,15,0.75,lower.tail=F))}`
5.  Decision and Interpretation <br> `Answer:` Since P-value \> 0.05
    then Fail to Reject $H_0$.
:::
...
-   `Interpretation`: Fail to Reject $H_0$. There is evidence to support
    ...
    from $75\%$ $p-value=0.787$; $95\%CI (0.27,0.79)$.
...
`1.DATA:` The data consist of observations on a bivariate random sample
$(X_i, Y_i)$, where $n'$ is the number of the `pairs`. If the X's and
Y's are independent, then the more powerful Mann-Whitney test is more
appropriate.
...
`2.Assumptions:` - The bivariate random variables $(X_i, Y_i)$ are
mutually independent . - The measurement scale is at least ordinal.

## Sign Test

`3.Test Statistic:` The test statistic is defined as follows:
...
`Solution:` $n'=18$ pairs, each is (RTBL,RTAL) = (X,Y)
...
-   `Interpretation`: Fail to Reject $H_0$. There is evidence to support
    that the data is compatible with the null hypothesis
    $p-value=0.071$; $95\%CI (0.48,1)$.
...
`Hypothesis: Two-tailed test` $$ H_0:p_1 = p_2 $$
$$H_1: p_1 \neq p_2 $$ - $p_1$ the probability that a randomly selected
obs from the population 1 will be in class 1.
...
`Hypothesis: Lower-tailed test` $$ H_0:p_1 = p_2 $$
$$H_1: p_1 < p_2 $$ - $p_1$ the probability that a randomly selected obs
from the population 1 will be in class 1.
...
`Hypothesis: Upper-tailed test` $$ H_0:p_1 = p_2 $$
$$H_1: p_1 > p_2 $$ - $p_1$ the probability that a randomly selected obs
from the population 1 will be in class 1.
...
::: r-fit-text
1.  Define the null and alternative hypotheses <br> `Answer:`
    $$ H_0: p_1 = p_2 $$ $$H_1: p_1 \neq p_2 $$ This is an two-tailed
    test.
2.  Find the Test statistic observed and null distribution<br> `Answer:`
    $T_{obs}=-1.2695$ and $T\sim N(0,1)$
3.  Determine critical values (rejection region) <br> `Answer:` +/-
    `r {qnorm(0.025)}`
4.  Find P-value<br> `Answer:` p-value=`r {2*pnorm(-1.2695)}`
5.  Decision <br> `Answer:` Since P-value \> 0.05 then Fail to Reject
    $H_0$.
:::
...
3.  Determine critical values (rejection region) <br> `Answer:` +/-
    `r {qnorm(0.025)}`
4.  Find P-value<br> `Answer:` p-value=`r {2*pnorm(-1.53)}`
5.  Decision and Interpretation <br> `Answer:` Since P-value \> 0.05
    then Fail to Reject $H_0$.
...
`Hypothesis: Lower-tailed test` $$ H_0:p_1 = p_2 $$
$$H_1: p_1 < p_2 $$ - $p_1$ the probability that a randomly selected obs
from the row 1 will be in col 1.
...
::: r-fit-text
1.  Define the null and alternative hypotheses <br> `Answer:`
    $$ H_0: p_1 \geq p_2 $$ $$H_1: p_1 < p_2 $$ This is an lower-tailed
    test.
2.  Find the Test statistic observed and null distribution<br> `Answer:`
    $T_{obs}=X=1$ and $T\sim hypergeometric(14,10,4)$\`
3.  Find P-value<br> `Answer:` p-value=`r {phyper(1,10,4,4)}`
4.  Decision and Interpretation <br> `Answer:` Since P-value \< 0.05
    then Reject $H_0$.
:::
...
-   `Interpretation`: Reject $H_0$. There is evidence to support that
    ...
    likely than males to get the account Rep. job.
